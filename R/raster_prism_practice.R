...
months <- list.files("PRISM_Data")
diff_list <- vector(mode = "list", length = length(months))

# For each month folder, find the t_diff and add it to the list
for (i in seq_along(months)) {
  files <- list.files(paste0("PRISM_Data/", months[i]))
  t_max_true <- str_detect(files, "tmax")
  t_max_file <- paste0("PRISM_Data/", months[i], "/", files[t_max_true])
  t_min_file <- paste0("PRISM_Data/", months[i], "/", files[!t_max_true])
