...
patient_almost <- read.table(
  text    = patient_txt,
  header  = FALSE,
  col.names = c("satisfaction", "age", "severity", "anxiety", "a", "b", "c", "d")
)
patient <- data.frame(satisfaction = c(patient_almost$satisfaction, patient_almost$a),
                      age = c(patient_almost$age, patient_almost$b),
                      severity = c(patient_almost$severity, patient_almost$c),
                      anxiety = c(patient_almost$anxiety, patient_almost$d))
...
summary
plot(patient_lm)
checkresiduals(patient_lm)
confint(patient_lm, level = 0.975)

# Look at variable importance of model
varImp(patient_lm)
